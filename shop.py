...
class Shop:
	...
	def PrintData(self):
		...
		try:
			...
			for row in results:
				idCategory = row[0]
				nameCategory = row[1]
				print("Category id: " + str(idCategory) + ", name: " + nameCategory)
			...
			for row in results:
				...
				priceProduct = row[3]
				amountProduct = row[4]
				print("Product id: " + str(idProduct) + ", Category id: " + str(idCategory) + ", name: " + nameProduct +
	...
	def CreateCategory(self, productCategory):
		...
		try:
			self.cursor.execute(sql)
			self.db.commit()
			print("Категорія %s успішно додана!" % productCategory.name)
			return True
		except:
			print("ПОМИЛКА! Категорія %s не додана !" % productCategory.name)
			self.db.rollback()
			return False
	...
	def CreateProduct(self, product):
		...
		try:
			self.cursor.execute(sql)
			self.db.commit()
			print("Продукт %s успішно доданий!" % product.name)
			return True
		except:
			print("ПОМИЛКА! Продукт %s не доданий!" % product.name)
			self.db.rollback()
			return False
	...
	def EditCategory(self, categoryId):
		...
		if(self.SearchCategory(categoryId, False)):
			...
			print("Введіть нове ім'я категорії (categoryName): ")
			categoryName = input()
			...
			try:
				self.cursor.execute(sql)
				self.db.commit()
				print("Категорія id = %d успішно відредагована!" % categoryId)
				return True
			except:
				print("ПОМИЛКА! Категорія id = %d не відредагована!" % categoryId)
				self.db.rollback()
				return False
	...
	def EditProduct(self, productId):
		...
		if(self.SearchProduct(productId, False)):
			...
			try:
				self.cursor.execute(sql)
				self.db.commit()
				print("Продукт id = %d успішно відредагований!" % productId)
				return True
			except:
				print("ПОМИЛКА! Продукт id = %d не відредагований!" % productId)
				self.db.rollback()
				return False
	...
	def DeleteCategory(self, сategoryId):
		...
		if (self.SearchCategory(сategoryId, False)):
			...
			try:
				self.cursor.execute(sql)
				self.db.commit()
				print("Категорія %s успішно видалена!" % delname)
				return True
			except:
				print("ПОМИЛКА при видаленні категорії %s" % delname)
				self.db.rollback()
				return False
	...
	def DeleteProduct(self, productId):
		...
		if(self.SearchProduct(productId, False)):
			...
			try:
				...
				print("Продукт %s успішно видалено!" % delname)
				return True
			except:
	...
	def SearchCategory(self, сategoryId, boolPrintOrNo):
		try:
			...
			results = self.cursor.fetchall()
			if len(results) == 0:
				print("Немає категорії з таким id = " + str(сategoryId))
			for row in results:
				idCategory = row[0]
				nameCategory = row[1]
				if boolPrintOrNo:
	...
	def SearchProduct(self, productId, boolPrintOrNo):
		try:
			...
			results = self.cursor.fetchall()
			if len(results) == 0:
				print("Немає продукту з таким id = " + str(productId))
			for row in results:
				idProduct = row[0]
				idCategory = row[1]
				nameProduct = row[2]
